...
<script type="text/javascript">
	...
	$(function(){
		...
		//给线索关联市场活动时，在文本框按下回车，搜索相应市场活动
		$("#aname").keydown(function (event) {
			if(event.keyCode==13)
			{
				$.ajax({
					url:"workbench/clue/getActivityListByNameNotClueID.do",
					data:{
						"name":$.trim($("#aname").val()),
						"clueId":"${clue.id}"
					},
					...
					success:function (data) {
						...
						$("#newActivityBody").html(html);
					}
				})

				//模态窗口在回车后会刷新页面，解决方法是哪里回车了，哪里加上return false
				return false;
			}
		})
		...
		$("#boundBtn").click(function () {
			...
			if($xz.length==0){
				...
				//当同一个key对应多个value时，自己字符串拼接param
				// "workbench/clue/bound.do?cid=xx&aid=xx&aid=xx&aid=xx"
				var param = "cid=${clue.id}&";
				for(var i=0;i<$xz.length;i++)
				{
					param += "aid="+$($xz[i]).val();//bom-->dom dom->bom
					if(i<$xz.length-1)
					{
						param += "&";
					}
				...
				$.ajax({
					...
					success:function (data) {
						if(data.success)
						{
							...
							$("#bundModal").modal("hide");//关掉模态窗口
							$("#aname").val("");//清除搜索框中的内容
							$("#newActivityBody").html("");//清空搜索出来的市场活动
							showActivityList();//刷新市场活动页面
						}else{
							alert("抱歉，关联失败，请重试");
						}
					}
	...
	//点击解除关联按钮，解除市场活动和线索之间的关联，即删除relation表中的对应记录
	//这里的id为relation表中的id
	function unbound(id) {
		if(confirm("是否要解除关联？"))
		{
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		<div class="page-header">
			<h3>${requestScope.clue.fullname}${requestScope.clue.appellation}<small>&nbsp;${clue.company}</small></h3>
		</div>
